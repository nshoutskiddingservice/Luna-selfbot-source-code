...
class luna:
    ...
    def register():
        ...
        try:
            ...
            files.remove('data/auth.json', documents=False)
            luna._extracted_from_register_10("There has been an issue with authenticating your hardware", "Redirecting to the main menu in 5 seconds » Code 5")
        ...
        try:
            ...
            luna._extracted_from_register_10(e, "Redirecting to the main menu in 5 seconds » Code 6")
                ...
                if ((("giveaway" in str(message.content).lower()) and (
                    ...
                    for blocked_word in giveaway_blocked_words:
                        if str(blocked_word).lower() in str(
                            ...
                            prints.sniper(
                                f"Reason  | Backlisted word » {color.print_gradient(f'{blocked_word}')}"
                            )
                            prints.sniper(
                                f"Server  | {color.print_gradient(f'{message.guild}')}"
                            )
                    ...
                    if found_something_blacklisted == 0:
                        try:
                            ...
                            prints.sniper(
                                f"Invite  | {color.print_gradient(f'Joined guild » {joined_server}')}"
                            )
                            print()
                            if files.json(
                                    "data/notifications/toasts.json",
    ...
    if not footer == "None":
        if footer_extra is None:
            if files.json(
                ...
                footer_extra = f"Enabled Protections » {active_protections} | {theme.footer()}"
            else:
